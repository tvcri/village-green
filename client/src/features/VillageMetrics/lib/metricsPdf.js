...
export const CHART_SIZE = 170

// A horizontal bar chart spends its width on y-axis labels before it draws any
// bar at all: at CHART_SIZE the Services chart had ~110pt of bar left after
// "Ride: Activity/Event". The legend can afford the difference — its three
// columns leave visible slack at CHART_SIZE — so bar mode takes a wider slot.
...
export function chartSlotWidth (chartType) {
  return chartType === 'bar' ? BAR_WIDTH : CHART_SIZE
}
const ROW_H = 15
...
// in MetricsChartCard.vue) but in points rather than CSS pixels. The padding
// covers the x-axis, its labels, and a little breathing room.
const PDF_BAR_THICKNESS = 22
const PDF_BAR_PADDING = 42

// Height of the drawn CHART. A pie is square, so CHART_SIZE. A bar chart sizes
...
export function chartDrawHeight (sliceCount, chartType) {
  if (chartType !== 'bar') return CHART_SIZE
...
//
// The CHART_SIZE floor lives in chartDrawHeight now rather than here. It is
// PIE-SPECIFIC: it exists so a pie card is never shorter than its square image.
// Applying it in bar mode left a 108pt chart and a 90pt legend inside a 170pt
// card, padding every short section with whitespace it had no use for — which
// is why bar mode is exempt.
...
function chartSectionHeight (view) {
...
// on-screen .chart-card. Returns the y a following section may start at.
async function drawChartSection (pdf, page, fonts, title, subtitle, image, view, y) {
  const { helv, bold } = fonts
  const cardH = chartSectionHeight(view)
  ...
  if (image) {
    ...
    // A pie draws as a CHART_SIZE square; a bar chart takes a wider slot and a
    ...
    page.drawImage(png, { x: chartX, y: inner - imgH, width: slotW, height: imgH })
  } else {
    drawText(page, view.emptyMessage, chartX, inner - CHART_SIZE / 2, 10, helv, MUTED)
  }

  const legendX = chartX + slotW + 24
...
export async function buildMetricsPdf (report) {
  ...
  y = await drawChartSection(pdf, p1, fonts, 'Categories',
  ...
  await drawChartSection(pdf, p1, fonts, 'Outcomes', '',
  ...
  const afterServicesY = await drawChartSection(pdf, p2, fonts, 'Services', svcSub,
