...
// SPEC — STALE: written against a flat /volunteers/{id} CRUD API that was never
// built. main implemented the volunteer role as a person sub-resource instead
// (PUT/PATCH/DELETE /persons/{personId}/volunteer), so these todos must be
// RETARGETED at those endpoints before they can flip green. Only GET /volunteers
// (the list) exists on the flat path. Auth/scope gating is GREEN in authz.test.js.
// Capability reference ids used below: 1=Errands, 2=Friends, 5=Rides.
const STUB = 'specced against the superseded flat /volunteers API; retarget to /persons/{id}/volunteer'
...
async function makeVolunteer (token, { label, capabilityIds } = {}) {
  const person = await vgFetch('/persons', { token, body: { villageId: quahog, firstName: 'Throwaway', lastName: `V${label}` } })
...
test('POST /volunteers creates a volunteer with capabilities', { todo: STUB }, async () => {
  const person = await vgFetch('/persons', {
    ...
    body: { villageId: quahog, firstName: 'Brand New', lastName: 'Volunteer' },
  })
