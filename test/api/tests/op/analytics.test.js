...
const SUMMARY = '/op/analytics/summary'

test('GET /op/analytics/summary as a non-admin -> 403', async () => {
  const { status } = await vgFetch(SUMMARY, { token: tokens.users.full_v1 })
  assert.equal(status, 403)
})

test('GET /op/analytics/summary as admin -> 200', async () => {
  // from/to are date-time (not date) per the schema.
  const { status } = await vgFetch(SUMMARY, {
    token: tokens.users.admin, query: { from: '2026-01-01T00:00:00Z', to: '2026-12-31T23:59:59Z' },
  })
