...
// SPEC (not yet implemented). Every Member handler is stubbed (`res.json({})`),
// so each assertion below is encoded as a `todo`: it is reported as a known
// failing test, does NOT fail the run, and flips to a real passing guard the
// moment the matching handler is implemented (drop the `todo` flag then).
// Auth/scope gating for these endpoints is verified GREEN in authz.test.js.
const STUB = 'Member.* handlers are stubbed (res.json({}))'
...
async function makeMember (token, { memberNumber } = {}) {
  ...
  const person = await vgFetch('/persons', { token, body: { villageId: quahog, fullName: `Throwaway ${memberNumber}` } })
...
test('POST /members creates a member and echoes the fields', { todo: STUB }, async () => {
  const person = await vgFetch('/persons', {
    ...
    body: { villageId: quahog, fullName: 'Brand New Member' },
  })
